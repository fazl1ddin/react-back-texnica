...
export const private_routes = [
    {
        path: '/',
        element: <CRUD path="/orders" hook={function (datas) {
        ...
        }} columns={orders} expanR={function (dataHook) {
    ...
    {
        path: "/products",
        element: <CRUD size={30} path="/products" columns={productsColumns} expandable={function () {
            return {
    ...
    {
        path: "/promos",
        element: <CRUD path="/promos" columns={promosColumns} expanR={() => (record) => {
            return <>
                <ImgXHR src={record.src} />
                <p className="text-lg">{record.title}</p>
                <p className="text-base" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(record.content) }} />
                <ul>
                    {
                        record.terms.map((item, index) => <li className="list-decimal list-inside" key={index}>{item}</li>)
                    }
                </ul>
            </>
        }} />,
    ...
    {
        path: "/news",
        element: <CRUD path="/news" s="-bigContent,-photo" columns={newsColumns} hook={function () {
            ...
            return (record) => {
                ...
                return <>
                    {
                        datas.loading ? <>afagaasga</> : <>
                            <ImgXHR src={element.photo} />
                            <p className="text-lg">{element.bigContent.ftitle}</p>
                            <p className="text-base" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(element.bigContent.fcontent)}}/>
                            <p className="text-lg">{element.bigContent.stitle}</p>
                            <p className="text-base" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(element.bigContent.scontent)}}/>
                        </>
                    }
                </>
            }
    ...
    {
        path: "/address-shops",
        element: <CRUD path="/address-shops" columns={ashColumns} hook={function (datas) {
            const { data, loading } = useGetDatasExper(
                "/cities",
                "POST",
